...
跳过**空白字符**。空白字符即ASCII小于等于32的字符。（我还特意查了ascii的表…）。_这里我可能会用isspace（掩面逃…）_
...
这里是parser的核心。判断该读入的元素类型。null，true，false这三个简单类型可以直接处理。其他的分别交给parse_number，parse_array和parse_object处理。ep是这里用于错误处理的指针，当出错的时候，ep指针里保存的就是当前出现错误的位置。
...
首先是遍历了一次整个string，统计出一共的字符个数，存在了len里，中间遇到转义的部分跳过了后面的字符。个数统计好之后，申请内存。这里的`cJSON_malloc`就是原本的`malloc`。在这个函数的定义出可以找到：
